  ...
  _process( chunk ) {
    ...
    while( offset < this.buffer.length ) {
      this.binary == false
        ...
        : hex = this.buffer.substring( offset, offset += 16 )
      ...
      this.binary == false
        ...
        : line += this.colors ? Hxd.colorHexCols( Hxd.binCols( hex ) ) : Hxd.binCols( hex )
...
// Will this cause some loss of performance? I don't know, need test
// I know that it fucking work
Hxd.binCols = function( hex ) {
  var i = 0
  return hex2bin(hex[i++]) + hex2bin(hex[i++]) + ' ' + hex2bin(hex[i++]) + hex2bin(hex[i++]) + ' ' +
    hex2bin(hex[i++]) + hex2bin(hex[i++]) + ' ' + hex2bin(hex[i++]) + hex2bin(hex[i++]) + ' ' +
    hex2bin(hex[i++]) + hex2bin(hex[i++]) + ' ' + hex2bin(hex[i++]) + hex2bin(hex[i++]) + ' ' +
    hex2bin(hex[i++]) + hex2bin(hex[i++]) + ' ' + hex2bin(hex[i++]) + hex2bin(hex[i++]) + ' ' +
    hex2bin(hex[i++]) + hex2bin(hex[i++]) + ' ' + hex2bin(hex[i++]) + hex2bin(hex[i++]) + ' ' +
    hex2bin(hex[i++]) + hex2bin(hex[i++]) + ' ' + hex2bin(hex[i++]) + hex2bin(hex[i++]) + ' ' +
